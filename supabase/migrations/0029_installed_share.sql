-- =============================================================================
-- ATLAS — Migration 0029 : la part des entreprises installées
--
-- Un domaine n'appartient pas qu'aux équipes de la salle : deux entreprises
-- installées y servent déjà une part du marché, et le facilitateur peut les
-- mettre en vente. Depuis qu'elles prélèvent leur part (migration applicative),
-- les parts des équipes ne somment plus à 100 % — ce qui est juste, mais
-- illisible tant que la part prélevée n'est nulle part.
--
-- Deux corrections, donc :
--
--   1. `installed_share` rejoint le résumé de pool. Σ parts + non servi +
--      installés = 1, et l'écran de révélation peut enfin nommer chaque
--      tranche de la barre au lieu de laisser un trou.
--   2. `unserved_share` change de base. La répartition rend son reliquat sur
--      la portion que les équipes SE DISPUTENT ; l'écran l'annonçait comme une
--      part du marché entier. Un marché aux installés à 30 % dont les équipes
--      laissent un dixième affichait 10 % de non-servi là où il n'en restait
--      que 7. La colonne porte désormais la même base que les parts.
--
-- Le corps de `atlas_persist_resolution_core` reproduit celui de la base,
-- augmenté de la seule colonne. Elle figure aussi dans la clause de conflit :
-- une résolution rejouée doit réécrire la part des installés.
-- =============================================================================

set search_path = atlas, public, extensions;

alter table pool_round_summary
  add column if not exists installed_share numeric not null default 0
    check (installed_share between 0 and 1);

comment on column pool_round_summary.installed_share is
  'Part du marché servie par les entreprises installées hors jeu. '
  'Σ parts des équipes + unserved_share + installed_share = 1.';
